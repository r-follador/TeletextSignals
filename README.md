...
### Text documents
Swiss news teletext articles in german language are used as an underlying source. They are downloaded from [teletext.ch](https://www.teletext.ch/SRF1/100).
...
is optimized for a) query retrieval, b) multi-language, and c) works well with news articles.
...
for examples). This is why a full text search (based on postgres' tsvector) is performed as an additional source for documents.
